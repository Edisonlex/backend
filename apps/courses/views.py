...
class AddCommentView(APIView):
    ...
    def post(self, request, course_uuid, *args, **kwargs):
        try:
            # Validar que el curso exista y esté publicado
            course = Course.postobjects.get(course_uuid=course_uuid)
            
            # Obtener y validar el contenido del comentario
            try:
            ...
            if len(content['message']) > 1000:  # puedes ajustar este límite
                return Response(
            ...
            if serializer.is_valid():
                # Guardar el comentario con el usuario actual
                comment = serializer.save(user=request.user.username)
                
                # Agregar el comentario al curso
            ...
            return Response(
                {'error': 'El curso no existe o no está publicado'},
...
class GetCourseData(APIView):
    def get(self, request, course_uuid, *args, **kwargs):
        if Course.objects.filter(course_uuid=course_uuid).exists():
            ...
            comments = course.comments.all().order_by('-created')
...
class GetCourseWhatlearnt(APIView):
    def get(self,request, course_uuid,*args, **kwargs):
        try:
            course = Course.objects.get(course_uuid=course_uuid)
            print("Course UUID:", course.course_uuid)
            print("Course title:", course.title)
            
            whatlearnt = course.what_learnt.all()
            print("WhatLearnt count:", whatlearnt.count())
            for item in whatlearnt:
                print("- WhatLearnt item:", item.title, "User:", item.user)
            ...
            print(f"No course found with UUID: {course_uuid}")
